...
def render_blender(blender_path, selected_files, yaml_dir):
    ...
    for file in selected_files:
        ...
        try:
            ...
            print("no existing files detected, skipping deletion")
        ...
        subprocess.run(
            [
                ...
                "JPEG",
                "-P",
                f"{yaml_dir}/make_blender_preview_eevee_settings.py",
                "-o",
                f"{output}#",
                "-f",
                "0",
    ...
    for file in selected_files:
        os.rename(
            (file[: file.rfind(".")]) + "0" + ".jpg", (file[: file.rfind(".")]) + ".jpg"
        )
...
if "blender" in ctx.inputs:
    ...
    if ap.check_application(
        blender_path, f"Path to Blender is not correct, please try again", "blender"
    ):
