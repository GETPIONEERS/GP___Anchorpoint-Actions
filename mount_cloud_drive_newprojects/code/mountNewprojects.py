...
def setup_mount(drive, workspace_id, configuration):
    ...
    if isWin():
        config_arguments.append(f"N:")  # CHANGE HERE: Drive Letter
    else:
...
def get_settings(workspace_id: str):
    ...
    shared_settings = aps.SharedSettings(
        workspace_id, "AnchorpointCloudMount_newprojects"
    )
    local_settings = aps.Settings("rclone_newprojects")
...
def on_application_started(ctx: ap.Context):
    try:
        shared_settings = aps.SharedSettings(
            ctx.workspace_id, "AnchorpointCloudMount_newprojects"
        )
