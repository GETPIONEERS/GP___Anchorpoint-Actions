...
def setup_mount(drive, workspace_id, configuration):
    def create_config_arguments():
        ...
        if (
            ...
            and configuration["type"] == "s3aws"
        ):
            config += [
                "--s3-location-constraint",
                f"{configuration['s3aws_location_constraint']}",
            ]
        ...
        if configuration["type"] == "azureblob":
            config += ["--azureblob-sas-url", f"{configuration['azureblob_sas_url']}"]
    ...
    base_arguments = [rclone_install._get_rclone_path(), "mount"]
    config_arguments = create_config_arguments()
    config_arguments.append(create_location_arguments())

    if isWin():
        config_arguments.append(f"P:")  # CHANGE HERE: Drive Letter
    else:
        if not os.path.isdir(drive):
            if "/Volumes" in drive:
                ...
                args = [
                    "/usr/bin/osascript",
                    "-e",
                    f'tell application "Finder" to make new folder at POSIX file "/volumes" with properties {properties}',
                ]
...
def dialog_setup_mount(dialog, workspace_id, configuration):
    if isWin():
        ...
        drive = os.path.normpath(
            os.path.join(dialog.get_value(path_var), mac_mount_name)
        )
...
def run_rclone(arguments, drive, workspace_id, startupinfo=None):
    ...
    for line in p.stdout:
        ...
        if (
            myjson != None
            and myjson["level"] == "error"
            and myjson["msg"] == "Mount failed"
        ):
            ...
            ui.show_error(
                ...
                duration=6000,
                description='Your settings do not seem to be correct. Go to the settings of "Connect Cloud Drive" and check if you have made a typing error.',
            )
            store_auto_mount(False, drive, workspace_id)
            return
            ...
            ui.show_error(
                title="Invalid Settings",
                duration=6000,
                description='Your Access Key seems to be wrong. Go to the settings of "Connect Cloud Drive" and check if you have made a typing error.',
            )
            store_auto_mount(False, drive, workspace_id)
            return
...
def check_upload(myjson, progress, count, count_uploaded):
    ...
    if not progress and percentage != "100%" and percentage != "-":
        progress = ap.Progress(
            "Syncing Files", percentage + " at " + upload_speed, infinite=False
        )
    ...
    if progress:
        if count > 1:
            ...
            progress.set_text(
                str(int((count_uploaded / count) * 100)) + "%" + " at " + upload_speed
            )
        else:
            if percentage == "-":
                percentage = "0"
...
def get_default_cache_path():
    if isWin():
        ...
        ap_cache_path = os.path.normpath(
            os.path.expanduser(
...
def get_settings(workspace_id: str):
    ...
    ui = ap.UI()
    shared_settings = aps.SharedSettings(workspace_id, "AnchorpointCloudMount_projects")
    local_settings = aps.Settings("rclone_projects")
    ...
    if shared_settings.get("Config") == "":
        if is_admin:
            ui.show_info(
                "No cloud drive configured",
                description="Please setup a cloud drive in your Workspace Settings inside the Actions entry.",
            )
        else:
            ui.show_info(
                "No cloud drive configured",
                description="Ask your workspace owner to setup a cloud drive",
            )
    else:
        ...
        if password == None:
            ...
            try:
                ...
                show_options(
                    shared_settings.get("mount_path"), workspace_id, configuration
                )
            except:
                create_pw_dialog(workspace_id)
...
def show_options(mount_path: str, workspace_id: str, configuration):
    ...
    if isWin():
        ...
        dialog.add_text("Drive Letter:\t").add_dropdown(
            drives[0], drives, var="drive_var"
        )
        dialog.add_button(
            "Mount",
        ...
        dialog.add_text("Drive Location:\t").add_input(
...
def on_application_started(ctx: ap.Context):
    try:
        shared_settings = aps.SharedSettings(
            ctx.workspace_id, "AnchorpointCloudMount_projects"
        )
        mount_settings = aps.Settings(ctx.workspace_id)
        if not mount_settings.contains(
            "rclone-automount"
        ) or not mount_settings.contains("rclone-drive"):
            return
...
if __name__ == "__main__":
    ctx = ap.Context.instance()
    ctx.run_async(
        rclone_install.check_winfsp_and_rclone, get_settings, ctx.workspace_id
    )
