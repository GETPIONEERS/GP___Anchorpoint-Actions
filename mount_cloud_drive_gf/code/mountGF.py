...
def setup_mount(drive, workspace_id, configuration):
    ...
    if isWin():
        config_arguments.append(f"G:")  # CHANGE HERE: Drive Letter
    else:
...
def get_settings(workspace_id: str):
    ...
    ui = ap.UI()
    shared_settings = aps.SharedSettings(workspace_id, "AnchorpointCloudMount_GF")
    local_settings = aps.Settings("rclone_gf")
...
def on_application_started(ctx: ap.Context):
    try:
        shared_settings = aps.SharedSettings(
            ctx.workspace_id, "AnchorpointCloudMount_GF"
        )
