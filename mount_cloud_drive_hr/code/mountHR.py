...
def setup_mount(drive, workspace_id, configuration):
    ...
    if isWin():
        config_arguments.append(f"H:")  # CHANGE HERE: Drive Letter
    else:
...
def get_settings(workspace_id: str):
    ...
    ui = ap.UI()
    shared_settings = aps.SharedSettings(workspace_id, "AnchorpointCloudMount_hr")
    local_settings = aps.Settings("rclone_hr")
...
def on_application_started(ctx: ap.Context):
    try:
        shared_settings = aps.SharedSettings(
            ctx.workspace_id, "AnchorpointCloudMount_hr"
        )
