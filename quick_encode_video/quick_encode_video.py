...
def button_encode(dialog):
    ...
    outputPath = folderPath + "/" + fileName + \
        "_QuickEncode" + ".mp4"  # build path for new file

    # encode video
    os.system(execPath + "\\ffmpeg.exe -i " + filePath + " -c:v libx264 -crf " +
              str(encodingQuality) + " -preset medium -b:v 0 " + outputPath)
...
dialog.add_text("Quality: ").add_dropdown("Select Quality",
                                          dropdownQualityValues, callback=dropdown_quality, var="dropdown_quality")
dialog.add_text("Custom CRF:").add_input(
    "30", callback=input_customQuality, var="input_customQuality")
dialog.add_info(
    "Default Quality settings are set with CRF values as follows:<br>Very High: 17 | High: 20 | Medium: 25 | Low: 35", var="info_quality")
...
dialog.add_switch(True, callback=switch_resolution,
                  var="switch_resolution").add_text("Keep Resolution")
dialog.add_text("Resolution: ").add_dropdown("Select Resolution",
                                             dropdownResolutionValues, callback=dropdown_resolution, var="dropdown_resolution")
dialog.add_button("Encode", callback=button_encode,
                  var="button_encode", enabled=False)
